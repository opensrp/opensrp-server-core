...
@Repository
public class AllProviders extends BaseRepository<Provider> {
	
	@Autowired
	protected AllProviders(@Qualifier(AllConstants.OPENSRP_DATABASE_CONNECTOR) CouchDbConnector db) {
	...
	public List<Provider> findAllProviders() {
		return db.queryView(createQuery("all_providers").includeDocs(true), Provider.class);
	}
	...
	public List<Provider> findAllProviderByIds(List<String> Ids) {
		return db.queryView(createQuery("all_providers_by_baseEntityIDs").keys(Ids).includeDocs(true), Provider.class);
	}
}
