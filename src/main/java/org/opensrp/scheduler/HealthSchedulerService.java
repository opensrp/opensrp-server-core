...
public class HealthSchedulerService {
	...
	public void alertFor(String windowName, String beneficiaryType, String entityId, String providerId, String schedule,
	        String milestone, DateTime startOfDueWindow, DateTime startOfLateWindow, DateTime startOfMaxWindow) {
		if (WindowName.late.toString().equals(windowName)) {
			actionService.alertForBeneficiary(beneficiaryType, entityId, providerId, schedule, milestone, urgent,
			    startOfLateWindow, startOfMaxWindow);
		} else if (WindowName.earliest.toString().equals(windowName)) {
			actionService.alertForBeneficiary(beneficiaryType, entityId, providerId, schedule, milestone, upcoming,
			    startOfDueWindow, startOfLateWindow);
		} else if (WindowName.due.toString().equals(windowName)) {
			actionService.alertForBeneficiary(beneficiaryType, entityId, providerId, schedule, milestone, normal,
			    startOfDueWindow, startOfLateWindow);
		} else if (WindowName.max.toString().equals(windowName)) {
			actionService.alertForBeneficiary(beneficiaryType, entityId, providerId, schedule, milestone, expired,
			    startOfMaxWindow, startOfMaxWindow);
		}
	}
	
	public void alertFor(String beneficiaryType, String entityId, String providerId, String schedule, String milestone,
	        AlertStatus alertStatus, DateTime startOfDueWindow, DateTime startDate, DateTime expiryDate) {
		actionService.alertForBeneficiary(beneficiaryType, entityId, providerId, schedule, milestone, alertStatus, startDate,
		    expiryDate);
	}
}
