...
public class AlertCreationAction implements HookedEvent {
	...
	public void invoke(MilestoneEvent event, Map<String, String> extraData) {
		...
		logger.debug(
		    format("Generating alert for entity {0} of type {1} , formSubmission {2} " + "for schedule {3} in window {4} ",
		        event.externalId(), entityType, fs.instanceId(), enr.getScheduleName(), event.windowName()));
		
		scheduler.alertFor(event.windowName(), entityType, event.externalId(), fs.anmId(), event.scheduleName(),
		    event.milestoneName(), event.startOfDueWindow(), event.startOfLateWindow(), event.startOfMaxWindow());
	}
