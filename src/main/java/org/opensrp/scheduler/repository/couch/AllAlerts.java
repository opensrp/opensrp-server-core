...
public class AllAlerts extends BaseRepository<Alert> implements AlertsRepository {
	...
	public List<Alert> findByProviderAndTimestamp(String provider, long timeStamp) {
		...
		return db.queryView(createQuery("alert_by_provider_and_time").startKey(startKey).endKey(endKey).includeDocs(true),
		    Alert.class);
	}
	...
	public List<Alert> findActiveByProviderAndTimestamp(String provider, long timeStamp) {
		...
		ComplexKey endKey = ComplexKey.of(provider, Long.MAX_VALUE);
		return db.queryView(
		    createQuery("alert_by_provider_and_time_active").startKey(startKey).endKey(endKey).includeDocs(true),
		    Alert.class);
	}
	
	@View(name = "alert_by_provider_entityId_trigger", map = "function(doc) { " + "if(doc.type === 'Alert') {"
	...
	@View(name = "alert_by_provider_entityId_trigger_active", map = "function(doc) { "
	        + "if(doc.type === 'Alert' && doc.isActive) {" + "emit([doc.providerId, doc.entityId, doc.triggerName], null)} "
	        + "}")
	public List<Alert> findActiveAlertByProviderEntityIdTriggerName(String provider, String entityId, String triggerName) {
		ComplexKey key = ComplexKey.of(provider, entityId, triggerName);
		return db.queryView(createQuery("alert_by_provider_entityId_trigger_active").key(key).includeDocs(true),
		    Alert.class);
	}
	
	@View(name = "alert_by_entityId_trigger_active", map = "function(doc) { " + "if(doc.type === 'Alert' && doc.isActive) {"
	...
	@View(name = "alert_by_entityId_active", map = "function(doc) { " + "if(doc.type === 'Alert' && doc.isActive) {"
	...
	public List<Alert> findByEntityIdTriggerAndTimeStamp(String entityId, String trigger, DateTime start, DateTime end) {
		...
		return db.queryView(
		    createQuery("alert_by_entityId_and_trigger_and_time").startKey(startKey).endKey(endKey).includeDocs(true),
		    Alert.class);
	}
	...
	public void markAlertAsCompleteFor(String entityId, String triggerName, String completionDate) {
		...
		if (existingAlerts.size() > 1) {
			logger.warn(MessageFormat.format(
			    "Found more than one alert for the combination of entityId: {0} and triggerName : {1}. Alerts : {2}",
