...
public class AllEnrollmentWrapper extends AllEnrollments {
	...
	public Enrollment getEnrollment(String externalId, String scheduleName) {
		...
		List<Enrollment> enrollments = queryView("find_by_external_id_and_schedule_name",
		    ComplexKey.of(externalId, scheduleName.toUpperCase()));
	...
	public List<Enrollment> findByEnrollmentDate(String status, DateTime start, DateTime end) {
		...
		List<Enrollment> enrollments = db
		        .queryView(createQuery("by_status_date_enrolled").startKey(s).endKey(e).includeDocs(true), Enrollment.class);
	...
	public List<Enrollment> findByLastUpDate(DateTime start, DateTime end) {
		List<Enrollment> enrollments = db
		        .queryView(createQuery("by_last_update").startKey(start).endKey(end).includeDocs(true), Enrollment.class);
		return populateWithSchedule(enrollments);
	}
