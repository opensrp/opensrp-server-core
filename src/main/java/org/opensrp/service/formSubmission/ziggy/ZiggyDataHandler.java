...
public class ZiggyDataHandler {
	...
	@Autowired
	public ZiggyDataHandler(@Qualifier(AllConstants.OPENSRP_DATABASE_CONNECTOR) CouchDbConnector db, EntityDataMap edMap) {
		this.db = db;
		this.edMap = edMap;
	}
	...
	List<ViewResult.Row> getDBViewQueryResult(String id, String entityType) {
		String docEntityType = edMap.getDocEntityType(entityType);
		return db.queryView(new ViewQuery().viewName(edMap.getIdViewName(entityType)).designDocId("_design/" + docEntityType)
		        .key(id).queryParam(edMap.getIdField(entityType), id).includeDocs(true)).getRows();
	}
