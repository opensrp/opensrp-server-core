...
public class PhysicalLocationService {
	...
	public void addOrUpdate(PhysicalLocation physicalLocation) {
		...
		if ((physicalLocation.isJurisdiction()
				&& getLocation(physicalLocation.getId(), DEFAULT_RETURN_BOOLEAN, false) == null)
	...
	@Transactional
	@CacheEvict(value = { "locationsWIthChildrenByLocationIds", "locationTreeFromLocation",
			"locationTreeFromLocationIdentifiers" }, allEntries = true)
	...
	public Set<String> saveLocations(List<PhysicalLocation> locations, boolean isJurisdiction) {
		...
		for (PhysicalLocation location : locations) {
			try {
				if (location.getProperties().getGeographicLevel() == 0 && !StringUtils
						.isEmpty(location.getProperties().getParentId())) {
	...
	public List<PhysicalLocation> findAllStructures(boolean returnGeometry, Long serverVersion, int limit,
			Integer pageNumber, String orderByType, String orderByFieldName) {
		return locationRepository
				.findAllStructures(returnGeometry, serverVersion, limit, pageNumber, orderByType, orderByFieldName);
	}
