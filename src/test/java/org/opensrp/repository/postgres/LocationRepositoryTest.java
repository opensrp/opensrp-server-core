...
public class LocationRepositoryTest extends BaseRepositoryTest {
	...
	protected Set<String> getDatabaseScripts() {
		...
		scripts.add("location_tag_map.sql");
		return scripts;
	}
	...
	public void testFindLocationsByPropertiesWithoutProperties() {
		...
		assertEquals(267, locations.get(0).getGeometry().getCoordinates().get(0).getAsJsonArray().get(0).getAsJsonArray()
		        .size());
	...
	public void testFindStructuresByProperties() {
		...
		assertEquals(2, locations.get(0).getGeometry().getCoordinates().get(0).getAsJsonArray().get(0).getAsJsonArray()
		        .size());
	...
	public void testFindAllLocationsByTagOrName() throws SQLException {
		...
		List<CustomPhysicalLocation> locs = new ArrayList<CustomPhysicalLocation>();
		locs = locationRepository.findLocationsByTagOrName("a", 0l, 20, 0);
		clientsArray = (JsonArray) gson.toJsonTree(locs, new TypeToken<List<CustomPhysicalLocation>>() {}.getType());
