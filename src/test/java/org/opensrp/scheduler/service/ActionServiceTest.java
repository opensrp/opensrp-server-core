...
public class ActionServiceTest {
	...
	public void shouldSaveAlertActionForEntity() throws Exception {
		...
		service.alertForBeneficiary("mother", "Case X", "ANM ID M", "Ante Natal Care - Normal", "ANC 1", normal, dueDate,
		    expiryDate);
		Action action = new Action("Case X", "ANM ID M",
		        ActionData.createAlert("mother", "Ante Natal Care - Normal", "ANC 1", normal, dueDate, expiryDate));
		
		verify(allActions).addOrUpdateAlert(action);
		verify(allAlerts).addOrUpdateScheduleNotificationAlert("mother", "Case X", "ANM ID M", "Ante Natal Care - Normal",
		    "ANC 1", normal, dueDate, expiryDate);
	}
	...
	public void shouldReportForIndicator() {
		ActionData summaryActionData = ActionData.reportForIndicator("ANC", "30",
		    new Gson().toJson(asList(new MonthSummaryDatum("3", "2012", "2", "2", asList("CASE 5", "CASE 6")))));
	...
	public void shouldCloseBeneficiary() throws Exception {
		...
		Action action = new Action(CASE_X, ANM_1,
		        ActionData.closeBeneficiary(BeneficiaryType.mother.name(), REASON_FOR_CLOSE));
		...
		Alert alert = new Alert(ANM_1, CASE_X, BeneficiaryType.mother.name(), Alert.AlertType.notification,
		        Alert.TriggerType.caseClosed, null, null, defaultDate, defaultDate, AlertStatus.urgent, null);
	...
	public void shouldReturnAlertsBasedOnANMIDAndTimeStamp() throws Exception {
		List<Action> alertActions = Arrays.asList(new Action("Case X", "ANM 1", ActionData.createAlert("mother",
