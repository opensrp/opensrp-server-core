...
public class HealthSchedulesTest extends TestResourceLoader {
	...
	public void testProperAlertRouting() {
		...
		sch.alertFor(late.name(), BENIFICIARY_TYPE, ENTITY_ID, PROVIDER_ID, SCHEDULE_NAME, MILESTONE, startOfDueWindow,
		    startOfLateWindow, startOfMaxWindow);
		verify(actionService).alertForBeneficiary(BENIFICIARY_TYPE, ENTITY_ID, PROVIDER_ID, SCHEDULE_NAME, MILESTONE, urgent,
		    startOfLateWindow, startOfMaxWindow);
		
		sch.alertFor(earliest.name(), BENIFICIARY_TYPE, ENTITY_ID, PROVIDER_ID, SCHEDULE_NAME, MILESTONE, startOfDueWindow,
		    startOfLateWindow, startOfMaxWindow);
		verify(actionService).alertForBeneficiary(BENIFICIARY_TYPE, ENTITY_ID, PROVIDER_ID, SCHEDULE_NAME, MILESTONE,
		    upcoming, startOfDueWindow, startOfLateWindow);
		
		sch.alertFor(due.name(), BENIFICIARY_TYPE, ENTITY_ID, PROVIDER_ID, SCHEDULE_NAME, MILESTONE, startOfDueWindow,
		    startOfLateWindow, startOfMaxWindow);
		verify(actionService).alertForBeneficiary(BENIFICIARY_TYPE, ENTITY_ID, PROVIDER_ID, SCHEDULE_NAME, MILESTONE, normal,
		    startOfDueWindow, startOfLateWindow);
		
		sch.alertFor(max.name(), BENIFICIARY_TYPE, ENTITY_ID, PROVIDER_ID, SCHEDULE_NAME, MILESTONE, startOfDueWindow,
		    startOfLateWindow, startOfMaxWindow);
		verify(actionService).alertForBeneficiary(BENIFICIARY_TYPE, ENTITY_ID, PROVIDER_ID, SCHEDULE_NAME, MILESTONE,
		    expired, startOfMaxWindow, startOfMaxWindow);
	}
	
	@Test
	public void testAlertRoutingWithAlertStatus() {
		sch.alertFor(BENIFICIARY_TYPE, ENTITY_ID, PROVIDER_ID, SCHEDULE_NAME, MILESTONE, upcoming, startOfDueWindow,
		    startOfLateWindow, startOfMaxWindow);
		verify(actionService).alertForBeneficiary(BENIFICIARY_TYPE, ENTITY_ID, PROVIDER_ID, SCHEDULE_NAME, MILESTONE,
		    upcoming, startOfLateWindow, startOfMaxWindow);
	}
	...
	public void testUnEnrollAndCloeSchedule() {
		sch.unEnrollAndCloseSchedule(ENTITY_ID, PROVIDER_ID, SCHEDULE_NAME, startOfDueWindow.toLocalDate(),
		    FORM_SUBMISSION_ID);
		verify(schService).unenroll(ENTITY_ID, SCHEDULE_NAME, FORM_SUBMISSION_ID);
		verify(actionService).markAlertAsClosed(PROVIDER_ID, ENTITY_ID, SCHEDULE_NAME,
		    startOfDueWindow.toLocalDate().toString());
	}
	...
	public void testFullFillMilestoneAndCloseAlert() {
		sch.fullfillMilestoneAndCloseAlert(ENTITY_ID, PROVIDER_ID, SCHEDULE_NAME, startOfDueWindow.toLocalDate(),
		    FORM_SUBMISSION_ID);
		verify(schService).fulfillMilestone(ENTITY_ID, SCHEDULE_NAME, startOfDueWindow.toLocalDate(), FORM_SUBMISSION_ID);
		verify(actionService).markAlertAsClosed(ENTITY_ID, PROVIDER_ID, SCHEDULE_NAME,
		    startOfDueWindow.toLocalDate().toString());
	}
	
	@Test
	public void testFullFillMilestoneAndCloseAlertWithMilestone() {
		sch.fullfillMilestoneAndCloseAlert(ENTITY_ID, PROVIDER_ID, SCHEDULE_NAME, MILESTONE, startOfDueWindow.toLocalDate(),
		    FORM_SUBMISSION_ID);
		verify(schService).fulfillMilestone(ENTITY_ID, SCHEDULE_NAME, startOfDueWindow.toLocalDate(), FORM_SUBMISSION_ID);
		verify(actionService).markAlertAsClosed(ENTITY_ID, PROVIDER_ID, MILESTONE,
		    startOfDueWindow.toLocalDate().toString());
	}
