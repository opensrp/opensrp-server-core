...
public class TaskSchedulerServiceTest {
	...
	@InjectMocks
	TaskSchedulerService taskSchedulerService = new TaskSchedulerService(motechSchedulerService, outboundEventGateway,
	        alertRouter);
	...
	public void testStartJobWithRepeatingSchedule() {
		...
		RepeatingSchedule repeatingSchedule = new RepeatingSchedule(SUBJECT, 5, TimeUnit.MILLISECONDS, 4,
		        TimeUnit.MILLISECONDS, new Date(1l), extraData);
		...
		verify(spyTaskSchedulingService).createRepeatingSchedulableJob(motechEvent, new DateTime(0l).plusMillis(5).toDate(),
		    new Date(1l), 4l);
	}
	...
	public void testStartJobWithRepeatingScheduleWithNullData() {
		RepeatingSchedule repeatingSchedule = new RepeatingSchedule(SUBJECT, 5, TimeUnit.MILLISECONDS, 4,
		        TimeUnit.MILLISECONDS, new Date(1l), null);
		TaskSchedulerService spyTaskSchedulingService = spy(taskSchedulerService);
		when(spyTaskSchedulingService.getCurrentTime()).thenReturn(new DateTime(0l));
	...
	public void testStartJobWithCornSchedule() {
		...
		RepeatingCronSchedule repeatingCronSchedule = new RepeatingCronSchedule(SUBJECT, 5, TimeUnit.MILLISECONDS,
		        cronExpression, new Date(1l), extraData);
		TaskSchedulerService spyTaskSchedulingService = spy(taskSchedulerService);
		when(spyTaskSchedulingService.getCurrentTime()).thenReturn(new DateTime(0l));
		...
		MotechEvent motechEvent = new MotechEvent(SUBJECT, extraData);
		CronSchedulableJob cronSchedulableJob = new CronSchedulableJob(motechEvent, cronExpression,
		        new DateTime(0l).plusMillis(5).toDate(), new Date(1l));
	...
	public void testStartJobWithCornScheduleWithNullData() {
		String cronExpression = "corn";
		RepeatingCronSchedule repeatingCronSchedule = new RepeatingCronSchedule(SUBJECT, 5, TimeUnit.MILLISECONDS,
		        cronExpression, new Date(1l), null);
		TaskSchedulerService spyTaskSchedulingService = spy(taskSchedulerService);
		when(spyTaskSchedulingService.getCurrentTime()).thenReturn(new DateTime(0l));
