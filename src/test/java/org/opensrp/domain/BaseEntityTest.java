...
public class BaseEntityTest {
	...
	public void testEqualAndHashcodeContract() {
		EqualsVerifier.forClass(BaseEntity.class).suppress(Warning.NONFINAL_FIELDS).suppress(Warning.STRICT_INHERITANCE)
		        .withPrefabValues(User.class, new User("dsd"), new User("sdf")).withRedefinedSuperclass().verify();
	}
	...
	public void testSuperObjectCreation() {
		...
		baseEntity.withCreator(creator).withDateCreated(dateCreated).withDateEdited(dateEdited).withDateVoided(dateVoided)
		        .withEditor(editor).withVoided(true).withVoidReason(voidReason).withVoider(voider);
