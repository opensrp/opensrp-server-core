...
public class FormSubmissionDataMigrationIntegrationTest extends BaseIntegrationTest {
	...
	public void shouldMigrateFormSubmissions() throws Exception {
		...
		List<AppStateToken> ol = allAppStateTokens
		        .findByName(AllConstants.Config.FORM_ENTITY_PARSER_LAST_MIGRATED_FORM_SUBMISSION.name());
		
		Assert.assertTrue("AppStateToken shouldn't be empty after a successful migration", !ol.isEmpty());
		Assert.assertTrue("AppStateToken should be greater than 0 all the times after a successful migration",
		    ol.get(0).longValue() > 0);
	...
	public void shouldMigrateFormSubmissionsAndPopulateTargetDB() throws Exception {
		...
		List<AppStateToken> ol = allAppStateTokens.findByName(Utils.connectToDB(sourceDb),
		    AllConstants.Config.FORM_ENTITY_PARSER_LAST_MIGRATED_FORM_SUBMISSION.name());
